...
#include <filesystem> // included for file iterating
#include <set>
...
void getParameters() {
    ...
    if (!ros::param::get("input_photo_path", photo_dir)) {
...
int main(int argc, char **argv) {
    ...
    for (auto &filename : sorted_by_name) {
        ...
        src_img = cv::imread(filename.c_str());

        if(src_img.empty()) {  // use the file name to search the photo
            cout << "No Picture found by filename: " << filename << endl;
            return 0;
        }
