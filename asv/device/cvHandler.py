...
class CVHandler(Node):
    ...
    def start_cv(self, file_name, callback, dummy_camera=None):
        ...
        self.subs[file_name] = self.create_subscription(String, f"auv/cv_handler/{file_name}", callback, 10)
...
class _ScriptHandler:
    def __init__(self, cv_handler, file_name, cv_object):
        ...
        self.pub_out = self.cv_handler.create_publisher(String, f"auv/cv_handler/{file_name}", 10)
        self.pub_cam_select = self.cv_handler.create_publisher(String, "/auv/camsVersatile/cameraSelect", 10)
