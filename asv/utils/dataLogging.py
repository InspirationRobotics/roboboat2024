...
class rosBags:
    def create_rosbag(self):
        ...
        rospy.Subscriber(
            ...
            self.bag_write_callback,
            callback_args=("/auv/devices/compass"),
        )
        rospy.Subscriber(
            "/auv/devices/imu",
            Imu,
            self.bag_write_callback,
            callback_args=("/auv/devices/imu"),
        )
        rospy.Subscriber(
            "/auv/devices/baro",
            std_msgs.msg.Float32MultiArray,
            self.bag_write_callback,
            callback_args=("/auv/devices/baro"),
        )
        rospy.Subscriber(
            "/auv/devices/thrusters",
            OverrideRCIn,
            self.bag_write_callback,
            callback_args=("/auv/devices/thrusters"),
        )
        rospy.Subscriber(
            "/auv/devices/setDepth",
            std_msgs.msg.Float64,
            self.bag_write_callback,
            callback_args=("/auv/devices/setDepth"),
        )
        rospy.Subscriber(
            "/auv/status/arm",
            std_msgs.msg.Bool,
            self.bag_write_callback,
            callback_args=("/auv/status/arm"),
        )
        rospy.Subscriber(
            "/auv/status/mode",
            std_msgs.msg.String,
            self.bag_write_callback,
            callback_args=("/auv/status/mode"),
        )
        rospy.spin()
