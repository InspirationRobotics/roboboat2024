...
class CV:
    """
    Notes for migration: only keep yaw because we live in 2 dimension.
    """
    ...
    def __init__(self, **config):
        """
        Sets up various attributes such as step, side, value, ratio, etc.
        Reads configuration parameters from the config dictionary.
        Sets the area based on the current sub (graey or onyx).
        Prints an initialization message.

        Parameters
        ----------
        config: dict
            Configuration parameters for the boat
        """
