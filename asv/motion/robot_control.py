...
class RobotControl:
    ...
    def set_depth(self, d):
        ...
        self.get_logger().info(f"[INFO] Depth set to {d}")
    ...
    def set_heading(self, target: int):
        ...
        target = (target) % 360
        self.get_logger().info(f"[INFO] Setting heading to {target}")

        while rclpy.ok():
            if self.compass is None:
                self.get_logger().warn("[WARN] Compass not ready")
                time.sleep(0.5)
                continue
        ...
        self.get_logger().info(f"[INFO] Finished setting heading to {target}")
    ...
    def navigate_dvl(self, x, y, z, end_heading=None, relative_coord=True, relative_heading=True, update_freq=10):
        ...
        end_heading (optional) is the heading to reach at the end of the navigation
        ...
        with self.dvl:
            ...
            while rclpy.ok():
                if not self.dvl.is_valid:
                    self.get_logger().warn("[WARN] DVL data not valid, skipping")
                    time.sleep(0.5)
                    continue
    ...
    def forwardHeading(self, power, t):
        ...
        t2 = 0
        self.get_logger().info("[INFO] finished backstopping")
        pwm.channels = [1500] * 18
        startTime = time.time()
        ...
        while time.time() - startTime <= 0.5:
            self.pub_thrusters.publish(pwm)
            time.sleep(0.05)
        self.get_logger().info("[INFO] finished forward heading")
    ...
    def forwardDist(self, dist, power):
        inches = 39.37 * dist
        self.get_logger.info(power, inches)
