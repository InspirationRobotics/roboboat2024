...
class GPS_Nav:
	...
	def imu_callback(self, data):
		...
		self.rotation = self.euler_from_quaternion(orientation)
		left = rospy.Publisher('/wamv/thrusters/left_thrust_cmd', Float32, queue_size=10)
		right = rospy.Publisher('/wamv/thrusters/right_thrust_cmd', Float32, queue_size=10)
		...
		print("Current Rotation : {0} radians / {1} degrees".format(self.rotation, math.degrees(self.rotation)))

		comp = math.degrees(self.rotation)
		comp=comp%360
		if(comp<0):
			comp=360+comp
		...
		angle_diff = self.target_angle - self.rotation
		...
		if self.arrived:
			angle_diff = self.keep_bearing - self.rotation
		...
		if ((angle_diff) > angle_thr):
			right.publish(speed1)
			left.publish(speed2)
			print("turning anticlockwise")
		elif ((angle_diff) < -angle_thr):
			left.publish(speed1)
			right.publish(speed2)
	...
	def gps_navigator(self):
		...
		self.target_lat = self.current_lat+1
		self.target_lon = self.current_lon
		self.station_keep = False
		self.keep_bearing = self.rotation
	...
	def position_hold(bearing, lat, long):
		...
		angle_thr = self.ANGLE_THR
		angle_diff = bearning - self.rotation
		if ((angle_diff) > angle_thr):
			right.publish(speed1)
			left.publish(speed2)
			print("turning anticlockwise")
